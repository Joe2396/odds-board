#!/usr/bin/env python3
"""
run_williamhill_props_FAST_PROD15_V22_ENGLAND_ALIAS_REPAIR_ONE_FILE.py

William Hill World Cup props scraper — FAST PROD15 V22 England alias repair — DR Congo/Congo DR panel matching.
...
Output:
  football/data/williamhill_worldcup_props_FAST_PROD15_V22_ENGLAND_ALIAS_REPAIR_CANDIDATE.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_PROD15_V22_ENGLAND_ALIAS_REPAIR_CANDIDATE.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_PROD15_V22_ENGLAND_ALIAS_REPAIR_CANDIDATE"

PREVIOUS_V18_PATH = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_PROD15_V18_CANDIDATE.json"
PREVIOUS_V19_PATH = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_PROD15_V19_RESUME_CANDIDATE.json"
PREVIOUS_V21_PATH = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_PROD15_V21_AVAILABILITY_CANDIDATE.json"
...
def v22_team_name_variants(team):
    """Return equivalent William Hill display-name variants for a team."""
    value = clean(team)
    ...
    # Reversed word order handles labels such as DR Congo / Congo DR.
    words = value.split()
    if len(words) == 2:
...
def v14_click_exact_away_text(page, away, heading_box):
    """V22: click an exact away label using aliases and token-order matching."""
    variants = v22_team_name_variants(away)
    target_token_key = v22_team_token_key(away)
    candidates = []

    for variant_index, variant in enumerate(variants):
        try:
            loc = page.get_by_text(
                re.compile(rf"^{re.escape(variant)}$", re.I),
                exact=False,
            )
            for i in range(min(loc.count(), 40)):
                ...
                try:
                    ...
                    item_text = clean(item.inner_text(timeout=1200))
                    item_token_key = v22_team_token_key(item_text)
                    if (
                        normalize(item_text) != normalize(variant)
                        and item_token_key != target_token_key
                    ):
                        continue

                    if heading_box:
                        min_y = heading_box["y"] + heading_box["height"] - 25
                        max_y = heading_box["y"] + heading_box["height"] + 300
                        if box["y"] < min_y or box["y"] > max_y:
                            continue
                    ...
                    candidates.append(
                        (score, item, box, i, variant, item_text)
                    )
                except Exception:
                    pass
        except Exception:
    ...
    for _, item, box, idx, variant, item_text in candidates:
        try:
            item.scroll_into_view_if_needed(timeout=1800)
            page.wait_for_timeout(180)
            item.click(force=True, timeout=3000)
            return True, {
                "method": "v22_alias_exact_text_force",
                "locator_index": idx,
                "matched_variant": variant,
                "visible_text": item_text,
                "box": box,
                "candidate_count": len(candidates),
            }
        except Exception as exc:
    ...
    return False, {
        "method": "v22_alias_exact_text_force",
        "variants": variants,
        "target_token_key": list(target_token_key),
        "candidate_count": len(candidates),
        "error": last_error,
    }
...
def v14_click_away_parent_box(page, home, away):
    """V22 fallback: token-order-tolerant away panel ancestor click."""
    js = r"""
        ({home, away}) => {
            const norm = s => (s || '')
                .toLowerCase()
            ...
            const tokenKey = s => norm(s)
                .split(' ')
                .filter(Boolean)
                .filter(x => !['the', 'of'].includes(x))
                .sort()
                .join('|');
            ...
            if (!headings.length) {
                return {status: 'heading_not_found'};
            }
            ...
            for (const el of nodes) {
                ...
                if (
                    er.top < hr.bottom - 25 ||
                    er.top > hr.bottom + 300
                ) continue;

                let cur = el;
                for (
                    let depth = 0;
                    depth < 9 && cur && cur !== document.body;
                    ...
                    if (
                        !text.split(' ').every(Boolean) ||
                        text.length > 180
                    ) continue;

                    // Away box may contain badges/counts, but must contain the
                    // away tokens and must not also contain all home tokens.
                    const awayTokens = tokenKey(A).split('|').filter(Boolean);
                    const homeTokens = tokenKey(H).split('|').filter(Boolean);
                    const textTokens = new Set(
                        norm(text).split(' ').filter(Boolean)
                    );

                    const hasAway = awayTokens.every(t => textTokens.has(t));
                    const hasHome = homeTokens.every(t => textTokens.has(t));

                    if (!hasAway || hasHome) continue;
                    if (
                        r.width < 160 || r.width > 1150 ||
                        r.height < 25 || r.height > 150
                    ) continue;
                    if (
                        r.top < hr.bottom - 25 ||
                        r.top > hr.bottom + 300
    ...
    try:
        ...
        if not result or result.get("status") != "candidate":
            return False, {
                "method": "v22_alias_away_parent_box",
                "result": result,
            }
...
def v14_click_right_half_of_team_row(page, home, away):
    """V22 final fallback: find both teams by unordered token containment."""
    js = r"""
        ({home, away}) => {
            const norm = s => (s || '')
                .toLowerCase()
            ...
            const containsTeam = (text, team) => {
                ...
                return tokens(team).every(token => set.has(token));
            };
    ...
    try:
        ...
        if not result or result.get("status") != "candidate":
            return False, {
                "method": "v22_alias_right_half_team_row",
                "result": result,
            }
...
def v17_click_scoped_show_more(page, heading, next_heading, max_clicks=30):
    """Click Show More only inside one exact William Hill market section.

    The section is bounded vertically by its exact heading and the next exact
    market heading. This prevents a Show More belonging to another accordion
    from being clicked.
    """
    clicks = 0
    stalled = 0
    ...
    for _ in range(max_clicks):
        result = page.evaluate(
            ...
            ({heading, nextHeading}) => {
                ...
                const nextWanted = norm(nextHeading);
                ...
                const nodes = Array.from(document.querySelectorAll(
                    'button,[role=button],a,div,span,p,h1,h2,h3,h4,h5'
                )).filter(styleVisible);

                const exactNodes = value => nodes
                    .filter(el => norm(el.innerText || el.textContent || '') === value)
                    .map(el => ({el, box: absBox(el)}))
                    .filter(x => x.box.width > 5 && x.box.height > 5)
                    .sort((a, b) =>
                        (a.box.width * a.box.height) - (b.box.width * b.box.height)
                    );

                const headingNodes = exactNodes(wanted);
                if (!headingNodes.length) {
                    return {status: 'heading_not_found'};
                }

                const headingItem = headingNodes[0];
                const headingTop = headingItem.box.top;
                const headingBottom = headingItem.box.bottom;

                const nextNodes = exactNodes(nextWanted)
                    .filter(x => x.box.top > headingTop + 2)
                    .sort((a, b) => a.box.top - b.box.top);

                const nextTop = nextNodes.length
                    ? nextNodes[0].box.top
                    : Number.POSITIVE_INFINITY;

                const showLabels = new Set(['show more', 'see more', 'view more']);
                const showCandidates = nodes
                    .filter(el => showLabels.has(norm(el.innerText || el.textContent || '')))
                    .map(el => {
                        const clickable =
                            el.closest('button,a,[role=button]') || el;
                        return {el: clickable, box: absBox(clickable)};
                    })
                    .filter(x =>
                        x.box.width > 5 &&
                        x.box.height > 5 &&
                        x.box.top > headingBottom &&
                        x.box.top < nextTop
                    )
                    .sort((a, b) => a.box.top - b.box.top);

                // A compact section signature lets Python detect a failed click.
                const sectionText = nodes
                    .map(el => {
                        const box = absBox(el);
                        if (box.top <= headingBottom || box.top >= nextTop) return '';
                ...
                if (!showCandidates.length) {
                    return {
                        status: 'complete',
                        signature: sectionText,
                        headingTop,
                        nextTop
                    };
                }

                const target = showCandidates[0].el;
                try {
                ...
                // Recalculate after scrolling, then use a real Playwright mouse
                // click at the returned viewport coordinates.
            ...
            """,
            {"heading": heading, "nextHeading": next_heading},
        )
        ...
        # Two unchanged passes normally means the visible button did not react.
        # Try a direct DOM click once before stopping.
        if stalled >= 2:
            direct = page.evaluate(
                ...
                ({heading, nextHeading}) => {
                    ...
                    const start = absTop(headings[0]);
                    const next = nodes
                        .filter(el =>
                            norm(el.innerText || el.textContent || '') === nextWanted &&
                            absTop(el) > start
                        )
                        .sort((a, b) => absTop(a) - absTop(b));
                    const end = next.length ? absTop(next[0]) : Infinity;

                    const show = nodes
                        .filter(el => ['show more', 'see more', 'view more']
                            .includes(norm(el.innerText || el.textContent || '')))
                        .map(el => el.closest('button,a,[role=button]') || el)
                        .filter(el => absTop(el) > start && absTop(el) < end)
                        .sort((a, b) => absTop(a) - absTop(b));
...
def v16_capture_players_tab_shots_tackles(page):
    """V17 override: exact Players markets plus scoped Show More expansion."""
    tab = try_tab_aliases(page, ["Players", "Player", "Player Stats"])
    if not tab:
        return [], {
            "status": "no_players_tab",
    ...
    targets = {
        "player_shots": {
            "aliases": ["Total Player Shots"],
            "next_heading": "Player Shot In Both Halves",
        },
        "player_tackles": {
            "aliases": ["Total Player Tackles"],
            "next_heading": "Player Fouls",
        },
    }
    ...
    # Process one exact accordion at a time so its own Show More is used.
    for key, spec in targets.items():
        heading, state = v11_open_first_exact(page, spec["aliases"])
        opened[key] = {
            "heading": heading,
            "state": state,
        }
        ...
        if heading:
            page.wait_for_timeout(300)
            show_more[key] = v17_click_scoped_show_more(
                page,
                heading,
                spec["next_heading"],
                max_clicks=30,
    ...
    # Give the final expanded DOM time to settle, then capture once.
    ...
    for key in targets:
        ...
        if market.get("selection_count", 0):
            for selection in market.get("selections", []):
                selection["source_top_tab"] = "Players"
                selection["source_price_mode"] = "players_tab_default"
                selection["source_expansion"] = "scoped_show_more"
            markets.append(market)
    ...
    print(
        "    Players scoped Show More: "
        f"Player Shots={show_more['player_shots'].get('clicks', 0)} clicks "
        f"({show_more['player_shots'].get('status')}), "
        f"Player Tackles={show_more['player_tackles'].get('clicks', 0)} clicks "
        f"({show_more['player_tackles'].get('status')})"
    )
...
def click_top_impact_sub_tab(page):
    """Robustly select the real top-level Impact Sub tab.

    Earlier versions incorrectly required Cards and Corners to be present in
    the same visible strip. Many valid event pages instead expose:
      Popular | Impact Sub | #YourOdds | Goals | Players | Half

    This function requires the candidate to share a compact navigation ancestor
    with Popular plus Goals or Players. It never selects per-market price-mode
    controls.
    """
    try:
        page.evaluate(
    ...
    find_js = r"""
        () => {
            ...
            const exact = Array.from(document.querySelectorAll(
                ...
                el => norm(el.innerText || el.textContent || '') === 'impact sub'
            );
            ...
            for (const label of exact) {
                const clickable =
                    label.closest('button,a,[role=tab],[role=button]') || label;
                const r = clickable.getBoundingClientRect();

                if (r.width < 28 || r.width > 380 ||
                    r.height < 14 || r.height > 120) {
                    continue;
                }
                ...
                for (let depth = 0; depth < 10 && cur; depth++, cur = cur.parentElement) {
                    ...
                    if (cr.height > 240 || cr.width < 280) continue;

                    const hasPopular = txt.includes('popular');
                    const hasGoals = txt.includes('goals');
                    const hasPlayers = txt.includes('players');
                    const hasHalf = txt.includes('half');
                    const hasYourOdds =
                        txt.includes('#yourodds') || txt.includes('your odds');

                    if (
                        hasPopular &&
                        (hasGoals || hasPlayers) &&
                        (hasHalf || hasYourOdds || (hasGoals && hasPlayers))
                    ) {
                        navEvidence = 1;
                        navText = txt.slice(0, 240);
                        navBox = cr;
                        break;
                    }
                }

                // Strongly prefer navigation-row evidence. A top-position
                // fallback is allowed only for compact tab-like elements.
                let score = 0;
                if (navEvidence) score += 10000;
                if (r.top >= 80 && r.top <= 700) score += 2000;
                if (r.width <= 180) score += 500;
                if (r.height <= 70) score += 300;
                if (clickable.matches('button,a,[role=tab],[role=button]')) {
                    score += 400;
                }

                // Per-market toggles tend to occur much lower on the page and
                // lack the Popular/Goals/Players navigation ancestor.
                if (!navEvidence && r.top > 720) continue;
                ...
                candidates.push({
                    ...
                    navEvidence,
                    navText,
                    ariaSelected: clickable.getAttribute('aria-selected') || '',
    ...
    for attempt in range(1, 5):
        ...
        if point:
            try:
                # Put the selected navigation element safely in view first.
                page.mouse.click(float(point["x"]), float(point["y"]))
            except Exception as exc:
                last_diag["click_error"] = f"{type(exc).__name__}: {exc}"
            else:
                # William Hill sometimes takes several seconds to hydrate the
                # selected tab on later fixtures.
                for wait_no in range(1, 9):
                    page.wait_for_timeout(700)
                    try:
                        body = page.locator("body").inner_text(timeout=15000)
                    except Exception:
                    ...
                    exact_lines = {normalize(x) for x in lines_from_text(body)}
                    verified = normalize("Scorer Markets") in exact_lines or any(
                        normalize(heading) in exact_lines
                        for aliases in V11_EXACT_PLAYER_HEADINGS.values()
                        for heading in aliases
                    )
        ...
        # Retry after a small viewport/layout reset. This also handles a tab
        # strip that finishes hydrating after the first query.
        try:
            page.evaluate(
                """
                () => {
    ...
    return False, {
        "status": "top_tab_not_found_or_unverified",
        "probe": last_diag,
    }
...
def click_top_impact_sub_tab(page):
    """V20 resilient top-level Impact Sub selector.

    This keeps the proven V19 navigation-row targeting, but adds longer
    hydration waits and a one-time reload when the tab text has not rendered.
    It is used only for the small repair set.
    """
    def visible_exact_points():
        return page.evaluate(
            ...
            () => {
                ...
                for (const label of nodes) {
                    ...
                    if (r.width < 25 || r.width > 420 ||
                        r.height < 12 || r.height > 130) {
                        continue;
                    }

                    let navEvidence = false;
                    let navText = '';
                    let cur = clickable.parentElement;
                    ...
                    for (let depth = 0; depth < 12 && cur; depth++, cur = cur.parentElement) {
                        ...
                        if (cr.height > 260 || cr.width < 260) continue;
                    ...
                    let score = 0;
                    if (navEvidence) score += 10000;
                    if (r.top >= 50 && r.top <= 760) score += 2000;
                    if (r.width <= 190) score += 500;
                    if (r.height <= 75) score += 300;
                    if (clickable.matches('button,a,[role=tab],[role=button]')) {
                        score += 500;
                    }

                    // Never accept a lower-page per-market price toggle unless
                    // it has the navigation-row evidence.
                    if (!navEvidence && r.top > 760) continue;
                ...
                return {
                    exactCount: nodes.length,
                    candidates: result.slice(0, 12)
                };
            }
            """
        )

    def impact_content_visible():
        try:
            body = page.locator("body").inner_text(timeout=15000)
        except Exception:
            return False
        ...
        exact_lines = {normalize(x) for x in lines_from_text(body)}
        if normalize("Scorer Markets") in exact_lines:
            return True
    ...
    for attempt in range(1, 11):
        ...
        page.wait_for_timeout(900 if attempt == 1 else 1400)

        try:
            diag = visible_exact_points() or {}
        except Exception as exc:
            diag = {
                "exactCount": 0,
                "candidates": [],
        ...
        for candidate_index, point in enumerate(candidates[:6], start=1):
            try:
                page.mouse.click(float(point["x"]), float(point["y"]))
            except Exception:
                continue

            for wait_cycle in range(1, 13):
                page.wait_for_timeout(650)
                if impact_content_visible():
                    return True, {
                        "status": "clicked_verified",
                        "attempt": attempt,
        ...
        # Portugal returned no exact nodes at all in V19. Give a genuinely
        # unhydrated page one clean reload before continuing.
        if attempt == 3 and not reloaded and not diag.get("exactCount"):
            try:
                ...
                last_diag["reload_error"] = f"{type(exc).__name__}: {exc}"

        # A final text-locator fallback is safe because we still verify that
        # Impact Sub market content appears after the click.
        try:
            loc = page.get_by_text("Impact Sub", exact=True)
            for i in range(min(loc.count(), 20)):
                item = loc.nth(i)
                box = item.bounding_box()
                if not box:
                    continue
                if (
                    box["width"] < 20 or box["width"] > 420 or
                    box["height"] < 10 or box["height"] > 140
                ):
                    continue
                if box["y"] > 800:
                    continue
                ...
                if impact_content_visible():
                    return True, {
                        "status": "playwright_text_verified",
                        "attempt": attempt,
                        "locator_index": i,
                        "reloaded": reloaded,
                        "probe": diag,
...
def scrape_match(page, fixture):
    ...
    if not tab_ok:
        # Some fixtures genuinely do not have an Impact Sub or Players tab yet.
        # Keep the useful Popular/default markets and mark player props as
        # unavailable rather than treating the fixture as a scraper failure.
        elapsed = round(time.perf_counter() - started, 1)

        debug_file = DEBUG_DIR / f"{slugify(name)}.txt"
        debug_file.write_text(
            "=== POPULAR DEFAULT (NO IMPACT SUB PUBLISHED) ===\n" + popular_text,
            encoding="utf-8",
        )
        save_hits(debug_file, popular_text)

        diag_path = DEBUG_DIR / f"{slugify(name)}_availability_diagnostics.json"
        diag_path.write_text(
            json.dumps({
                "impact_sub_available": False,
                "top_impact_sub_tab": tab_diag,
                "reason": "top_level_impact_sub_tab_not_published",
            }, indent=2, ensure_ascii=False),
            encoding="utf-8",
        )

        print("    ℹ No top-level Impact Sub tab published for this fixture")
        print("    ✓ Keeping Popular/Default markets only")
        print(f"  ✓ {home} v {away} — {len(markets)} markets in {elapsed}s")
        print("    player coverage: 0 markets / 0 selections")
        for market in markets:
            print(
                f"      {market['market']:<30} "
        ...
        return {
            ...
            "url": url,
            "url_source": fixture.get("url_source", ""),
            "captured_tabs": ["Popular/Default"],
            "impact_sub_available": False,
            "availability_reason": "top_level_impact_sub_tab_not_published",
            "player_price_mode": "not_available",
            "capture_strategy": "v22_alias_tolerant_scorer_panel_repair",
            "diagnostics": str(diag_path),
            "market_count": len(markets),
            "player_market_count": 0,
...
def _load_v21_output():
    if not PREVIOUS_V21_PATH.exists():
        raise RuntimeError(
            "Missing V21 candidate JSON. Run the V21 availability candidate first:\n"
            f"{PREVIOUS_V21_PATH}"
        )

    data = json.loads(PREVIOUS_V21_PATH.read_text(encoding="utf-8"))
    rows = data.get("matches", []) if isinstance(data, dict) else []
    if not isinstance(rows, list):
        raise RuntimeError("V21 candidate JSON has no matches list")
    return data, rows


def _market_map(fixture):
    result = {}
    for market in fixture.get("markets", []):
        if not isinstance(market, dict):
            continue
        key = normalize(market.get("normalized_market") or market.get("market"))
        if key == "anytime_goalscorer":
            key = "anytime_scorer"
        elif key in {"to_score_2", "to_score_2_plus"}:
            key = "scorer_2_plus"
        elif key in {"player_shown_a_card", "player_card"}:
            key = "player_cards"
...
def _scorer_panel_count(market):
    return len({
        clean(row.get("source_team_panel")).lower()
        for row in market.get("selections", [])
        if clean(row.get("source_team_panel"))
    })


def _repair_reason(fixture):
    if clean(fixture.get("error")) or fixture.get("market_count", 0) <= 0:
        return "scraper_error_or_no_markets"

    if fixture.get("impact_sub_available") is False:
        return ""

    markets = _market_map(fixture)
    for key in ["first_goalscorer", "anytime_scorer", "scorer_2_plus"]:
        market = markets.get(key)
        if not market:
            return f"missing_{key}"
        if _scorer_panel_count(market) < 2:
            return f"single_scorer_panel_{key}"

    return ""


def _fixture_for_retry(row):
    ...
    return {
        ...
        "url_source": "v22_alias_repair",
    }


def _scrape_with_retries(browser, fixture, max_attempts=3):
    last_error = ""
    best_result = None
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            last_error = f"{type(exc).__name__}: {exc}"
            print(f"    repair attempt failed: {last_error}")
            page.close()
            continue

        page.close()

        reason = _repair_reason(result)
        if not reason:
            return result

        print(f"    repair result still invalid: {reason}")
        best_result = result

    if best_result is not None:
        best_result["error"] = (
            clean(best_result.get("error"))
            or f"Repair validation failed after {max_attempts} attempts: "
               f"{_repair_reason(best_result)}"
        )
        return best_result
    ...
    return {
        ...
        "elapsed_seconds": 0,
        "markets": [],
        "error": last_error or "All repair attempts failed",
    }


def _selection_duplicate_key(row):
    # Include the canonical selection label. This prevents Yes/No or
    # Over/Under selections at the same odds being falsely marked duplicate.
    return (
        normalize(row.get("selection")),
        clean(row.get("player")).lower(),
        clean(row.get("threshold")).lower(),
        clean(row.get("line")).lower(),
        normalize(row.get("prop_type")),
        clean(row.get("odds")).upper(),
    )


def _fingerprint(market):
    return {
        (
            normalize(row.get("selection")),
            clean(row.get("player")).lower(),
...
def _validate_v22(output):
    ...
    impact_available_count = 0
    popular_only_count = 0

    for fixture in fixtures:
        name = clean(fixture.get("match")) or "<unknown fixture>"

        if clean(fixture.get("error")):
            failures.append(f"{name}: scraper error: {fixture.get('error')}")

        markets = _market_map(fixture)
        if not markets:
            failures.append(f"{name}: no markets captured")
            continue

        explicit_availability = fixture.get("impact_sub_available")
        if explicit_availability is None:
            # Preserved V19 results predate the explicit flag. Existing scorer
            # markets prove that Impact Sub was available on those fixtures.
            impact_available = any(
                key in markets
                for key in [
                    "first_goalscorer",
                    "anytime_scorer",
            ...
            impact_available = bool(explicit_availability)

        if impact_available:
            impact_available_count += 1

            for key in scorer_coverage:
                market = markets.get(key)
                if not market:
                    failures.append(f"{name}: missing scorer market {key}")
                    continue

                scorer_coverage[key] += 1
                panel_count = _scorer_panel_count(market)
                if panel_count < 2:
                    failures.append(
                        f"{name}: {key} contains only "
                        f"{panel_count} scorer panel(s)"
                    )

                if len(market.get("selections", [])) < 20:
                    failures.append(
                        f"{name}: {key} has only "
                        f"{len(market.get('selections', []))} selections"
                    )
        else:
            popular_only_count += 1
            warnings.append(
                f"{name}: no Impact Sub tab currently published; "
                "Popular/default markets retained"
            )

        for key in optional_coverage:
            if key in markets:
                optional_coverage[key] += 1

        for key, market in markets.items():
            rows = market.get("selections", [])
            if market.get("selection_count") != len(rows):
                failures.append(
                    f"{name}: {market.get('market')} selection_count mismatch"
                )

            seen = set()
            for row in rows:
                duplicate_key = _selection_duplicate_key(row)
                if duplicate_key in seen:
                    failures.append(
                        f"{name}: exact duplicate in "
                        f"{market.get('market')}: "
                        f"{row.get('selection')} {row.get('odds')}"
                    )
                    break
                ...
                if match:
                    ...
                    try:
                        actual = float(clean(row.get("line")))
                    except Exception:
                        failures.append(
                            f"{name}: invalid line for "
                            f"{row.get('player')} {threshold}"
                        )
                        break
                    if abs(expected - actual) > 1e-9:
                        failures.append(
                            f"{name}: {row.get('player')} {threshold} "
                            f"expected line {expected:g}, got {actual:g}"
                        )
                        break

        for key in ["player_shots", "player_tackles"]:
            market = markets.get(key)
            if market and 1 <= len(market.get("selections", [])) <= 10:
                failures.append(
                    f"{name}: {key} looks unexpanded "
                    f"({len(market.get('selections', []))} selections)"
                )

        fouls_committed = markets.get("player_fouls_committed")
        fouls_won = markets.get("player_fouls_won")
        if (
            fouls_committed and fouls_won and
            _fingerprint(fouls_committed) == _fingerprint(fouls_won)
        ):
            failures.append(
                f"{name}: Fouls Committed and Fouls Won are exact copies"
            )

        shots = markets.get("player_shots")
        sot = markets.get("player_shots_on_target")
        if shots and sot and _fingerprint(shots) == _fingerprint(sot):
            failures.append(
                f"{name}: Player Shots and SOT are exact copies"
            )

    for key, count in scorer_coverage.items():
        if count != impact_available_count:
            failures.append(
                f"Scorer coverage mismatch: {key} appears on {count}/"
                f"{impact_available_count} Impact Sub fixtures"
            )
...
def main():
    ...
    print("=" * 72)
    print("William Hill FAST PROD15 V22 — ENGLAND SCORER PANEL REPAIR")
    print("Preserves V21 and retries only alias-related scorer-panel failures")
    print("Production williamhill_worldcup_props.json will NOT be changed")
    print("=" * 72)

    previous_data, previous_rows = _load_v21_output()
    if len(previous_rows) != 15:
        raise RuntimeError(
            f"Expected 15 V21 fixture records, found {len(previous_rows)}"
        )

    repair_indexes = [
        index
        for index, row in enumerate(previous_rows)
        if _repair_reason(row)
    ]

    print(f"V21 fixtures needing repair: {len(repair_indexes)}")
    for index in repair_indexes:
        row = previous_rows[index]
        print(
            f"  - {row.get('match')}: {_repair_reason(row)}"
        )

    results = list(previous_rows)
    started = time.perf_counter()
    ...
    if repair_indexes:
        with sync_playwright() as p:
            ...
            for repair_number, index in enumerate(repair_indexes, start=1):
                existing = previous_rows[index]
                fixture = _fixture_for_retry(existing)

                print()
                print("=" * 72)
                print(
                    f"[repair {repair_number}/{len(repair_indexes)}] "
                    f"{fixture['name']}"
                )
                results[index] = _scrape_with_retries(
                    browser,
                    fixture,
                    max_attempts=2,
                )
    ...
    output = {
        ...
        "matches_with_markets": sum(
            row.get("market_count", 0) > 0 for row in results
        ),
        "total_player_markets": sum(
            row.get("player_market_count", 0) for row in results
        ),
        "total_player_selections": sum(
            row.get("player_selection_count", 0) for row in results
        ),
        "elapsed_seconds": elapsed,
        "repair_source": str(PREVIOUS_V21_PATH),
        "capture_strategy": "v22_alias_tolerant_scorer_panel_repair",
        "matches": results,
    }
    ...
    print("── V22 summary ────────────────────────────────────────────────")
    for row in results:
        markets = _market_map(row)
        optional_count = sum(
            key in markets for key in [
                "player_shots_on_target",
                "player_shots",
                "player_tackles",
                "player_fouls_committed",
        ...
        status = "OK" if not _repair_reason(row) else "FAIL"
        print(
            f"  [{status:<4}] {row.get('match', ''):<38} "
            f"{row.get('market_count', 0):>2} markets | "
            f"{optional_count}/7 optional stats | "
            f"{row.get('player_selection_count', 0):>4} player selections"
        )

    print(f"Repair runtime: {elapsed}s")
    print(f"Saved candidate: {OUT_PATH}")

    print()
    print("=" * 72)
    print("Running availability-aware V22 validation")
    print("=" * 72)
    ...
    (
        ...
        impact_available_count,
        popular_only_count,
    ) = _validate_v22(output)

    print(
        f"Impact Sub availability: {impact_available_count}/15 fixtures "
        f"({popular_only_count} Popular-only)"
    )
    print("Required scorer coverage on Impact Sub fixtures:")
    for key, count in sorted(scorer_coverage.items()):
        print(
            f"  {key:<30} "
            f"{count:>2}/{impact_available_count}"
        )

    print()
    print("Optional player-stat availability:")
    for key, count in sorted(optional_coverage.items()):
        print(f"  {key:<30} {count:>2}/15")

    report = {
        "status": "PASS" if not failures else "FAIL",
        "validated_at": datetime.now(timezone.utc).isoformat(),
        "input": str(OUT_PATH),
        "impact_sub_available_fixtures": impact_available_count,
        "popular_only_fixtures": popular_only_count,
        "required_scorer_coverage": scorer_coverage,
        "optional_market_availability": optional_coverage,
        "warnings": warnings,
        "failures": failures,
    }
    report_path = DEBUG_DIR / "validation_report.json"
    ...
    if failures:
        ...
        for failure in failures[:60]:
            print(f"  - {failure}")
        print("Existing production remains untouched.")
        return 1

    print("VALIDATION PASS")
    print("All fixtures contain every market William Hill currently publishes.")
    print("Existing production remains untouched.")
    return 0
