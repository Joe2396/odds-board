#!/usr/bin/env python3
# BETVICTOR_PLAYER_STATS_EXACT_FAST_TEST3_V7_SCOPED_FAST
"""
...
Output:
    football/data/betvictor_player_stats_exact_fast_test_v7_scoped_fast.json
...
OUT_PATH = DATA_DIR / "betvictor_player_stats_exact_fast_test_v7_scoped_fast.json"
DEBUG_ROOT = ROOT / "football" / "debug" / "betvictor_player_stats_exact_fast_test_v7_scoped_fast"
...
def scoped_show_more_locator(page):
    root = page.locator(
        "[data-bv-exact-market-scope='1']"
    )
...
def click_scoped_show_more_fast(
    ...
    """
    Click Show More inside only the marked market card until it disappears.
    No whole-page scrolling is performed during the normal successful path.
    """
    clicks = 0
    passes = []
    pattern_source = config["row_re"].pattern
    ...
    for pass_number in range(
        ...
        control = scoped_show_more_locator(page)
        ...
        try:
            page.wait_for_function(
                ...
                timeout=SHOW_MORE_GROWTH_TIMEOUT_MS,
            )
        except Exception:
            page.wait_for_timeout(
        ...
        # Expansion can replace the card subtree. Re-mark it before extracting.
...
def find_scoped_show_more(page, heading_labels, pattern_source):
    """
    Find an exact Show More button belonging to the current market card.

    A button belongs to this market only when one of its nearby ancestors
    contains at least one individual line matching this market's player-row
    regex. Testing line-by-line is essential because the regex is anchored.
    """
    candidates = []
...
def click_scoped_show_more_until_done(
    ...
    for pass_number in range(
        ...
        # The list expands vertically. Scroll the window through the new rows
        # after each click, but do not click any other controls.
        harvest_window(
            page,
            extractor,
            store,
        ...
        # Give one extra wait when the DOM updated slowly.
        if added == 0:
...
def main():
    ...
    print("BETVICTOR EXACT PLAYER STATS — FAST TEST3 V7 SCOPED FAST")
    print("=" * 72)
