#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_match_stats_FAST_TEST3_CORNERS_CARDS_V6_SPEED_PASS.py

FAST TEST3 match-stats V6: reduced waits, resource blocking, fast team tabs.
...
OUT_PATH = ROOT / "football" / "data" / "williamhill_worldcup_match_stats_FAST_TEST3_CORNERS_CARDS_V6_SPEED_PASS.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_match_stats_FAST_TEST3_CORNERS_CARDS_V6_SPEED_PASS"
...
def scrape_tab_market(page, tab_name, heading_candidates, market_name, prop_type, debug_chunks):
    """V5 fast path: one proven heading, one anchored parse, one tab retry."""
    started = time.perf_counter()

    preferred_heading = (
        "Match Over/Under Corners"
        if normalize(tab_name) == "corners"
        else "Match Over/Under Cards"
    )

    # First attempt.
    clicked_tab = click_tab(page, tab_name)

    # William Hill occasionally hydrates a later tab slowly. Retry the same
    # tab once rather than running all aliases or abandoning the market.
    retried_tab = False
    if not clicked_tab:
        retried_tab = True
        page.wait_for_timeout(1200)
    ...
    print(
        f"      {'✓' if clicked_tab else '-'} {tab_name} tab"
        + (" (retry)" if retried_tab else "")
    )

    if not clicked_tab:
        debug_chunks.append(
            f"\n\n=== V5 TAB {tab_name} ABSENT ===\n"
            + json.dumps(
                {
                    "preferred_heading": preferred_heading,
                    "tab_retried": retried_tab,
    ...
    if not opened_heading:
        # One short hydration retry for the same exact market only.
        page.wait_for_timeout(700)
        opened_heading, open_diag_retry = v16_open_exact_market(
            page,
            [preferred_heading],
        )
        open_diag = {
            "first": open_diag,
            "retry": open_diag_retry,
        }

    if not opened_heading:
        print(
            f"      - exact {preferred_heading} not published"
        )
        debug_chunks.append(
            f"\n\n=== V5 TAB {tab_name} / HEADING ABSENT ===\n"
            + json.dumps(
                {
                    "preferred_heading": preferred_heading,
                    "open": open_diag,
    ...
    print(f"      clicked exact {opened_heading}")
    ...
    # The V4 run proved the complete ladder is already present. Read it once
    # with the anchored text parser that returned 12-14 correct selections.
    page.wait_for_timeout(350)
    try:
    ...
    market = parse_tab_ou_market_from_text(
        body_text,
        [opened_heading],
        market_name,
        prop_type,
    ...
    # Only one same-heading retry for an empty lazy-render result.
    lazy_retry = False
    if market["selection_count"] == 0:
        lazy_retry = True
        page.wait_for_timeout(700)
        try:
            body_text_retry = page.locator("body").inner_text(
                timeout=10000
            )
        except Exception:
            body_text_retry = ""

        retry_market = parse_tab_ou_market_from_text(
            body_text_retry,
            [opened_heading],
            market_name,
            prop_type,
...
def fast_body_text(page):
    """Faster than locator('body').inner_text() for repeated parser snapshots."""
    try:
        return page.evaluate(
...
def click_tab(page, tab_name):
    """V6 faster top-tab switch with one short fallback."""
    try:
        page.evaluate(
            "window.scrollTo(0,0);"
    ...
    for role in ["link", "button", "tab"]:
        try:
            loc = page.get_by_role(
                role,
                name=re.compile(f"^{re.escape(tab_name)}$", re.I),
            )
            if loc.count():
...
def open_single_market(page, heading):
    """V6 fast accordion open; these markets do not need Show More."""
    ok = click_accordion_heading(page, heading)
    close_login_popups(page)
    print(f"      {'clicked' if ok else 'missed '} {heading}")
    if ok:
...
def click_visible_team_tab(page, team, heading=""):
    """V6 fast team-tab click with a shorter post-click settle."""
    terms = team_search_terms(team)
    ...
    js_textnode = r"""
        ({terms}) => {
            ...
            const exactTeam = txt =>
                lows.some(t => norm(txt).toLowerCase() === t);
            ...
            while ((node = walker.nextNode())) {
                ...
                for (
                    ...
                    candidates.push({
                        ...
                        score:
                            Math.abs(
                                cr.top - window.innerHeight * 0.42
                            ) +
                            cr.width * cr.height * 0.001,
                        text: elText.slice(0, 120)
                    });
                    break;
                }
            }
...
def scrape_tab_market(page, tab_name, heading_candidates, market_name, prop_type, debug_chunks):
    """V6 fast direct full-ladder parser with short count polling."""
    started = time.perf_counter()
    ...
    if not opened_heading:
        ...
        opened_heading, retry_diag = v16_open_exact_market(
            page,
            [preferred_heading],
        )
        open_diag = {
    ...
    if not opened_heading:
        print(f"      - exact {preferred_heading} not published")
        return None
    ...
    expected_minimum = 10 if prop_type == "corners" else 12
    best_market = mkt(market_name, [])
    snapshots = []

    # Full ladders hydrate progressively. Poll the same already-open market
    # instead of reopening headings or running alternate-market loops.
    for wait_ms in [250, 350, 500, 700]:
        page.wait_for_timeout(wait_ms)
        body_text = fast_body_text(page)
        candidate = parse_tab_ou_market_from_text(
            body_text,
            [opened_heading],
            market_name,
            prop_type,
        ...
        if best_market["selection_count"] >= expected_minimum:
            break

    elapsed = round(time.perf_counter() - started, 1)

    print(
        f"        parsed {best_market['selection_count']} selections "
        f"in {elapsed}s"
    )

    debug_chunks.append(
        f"\n\n=== MATCH-STATS V6 TAB {tab_name} ===\n"
        + json.dumps(
            {
                "preferred_heading": preferred_heading,
                "tab_retried": retried_tab,
                ...
                "snapshots": snapshots,
                "selection_count": best_market["selection_count"],
                "elapsed_seconds": elapsed,
            },
            indent=2,
            ensure_ascii=False,
    ...
    if best_market["selection_count"] > 0:
        return best_market
...
def scrape_match_stats(page, fixture):
    ...
    print(
        f"  URL source: "
        f"{fixture.get('url_source', 'unknown')}"
    )
    ...
    # Images/fonts are blocked by the browser context. Three seconds has been
    # enough for the navigation and market tabs to hydrate in prior runs.
    ...
    for heading, variants, market_name, prop_type in match_sections:
        ...
        text_snapshot = fast_body_text(page)
    ...
    if (
        corners_market and
        corners_market["selection_count"] > 0
    ):
    ...
    for heading, variants, base_name, prop_type in team_sections:
        opened = open_single_market(page, heading)
        if not opened:
            continue
        ...
        for team, side in [(home, "home"), (away, "away")]:
            ...
            # One fallback only. The old code could reopen/click three times.
            if not tab_ok or market["selection_count"] == 0:
            ...
            print(
                f"        "
                f"{'clicked' if tab_ok else 'missed '} "
                f"{team} tab for {heading}"
            )
    ...
    timing = {
        "navigation_seconds": round(
            navigation_ready_at - match_started,
            1,
        ),
        "match_sections_seconds": round(
            match_sections_done_at - navigation_ready_at,
            1,
        ),
        "corners_cards_seconds": round(
            top_tabs_done_at - match_sections_done_at,
            1,
        ),
        "team_sections_seconds": round(
            finished_at - top_tabs_done_at,
            1,
        ),
    ...
    print(
        f"  ✓ {home} v {away} — "
        f"{len(unique)} stat markets in {elapsed}s"
    )
    print(
        "    timing: "
...
def main():
    ...
    print("=" * 60)
    print("William Hill Match/Team Stats — FAST TEST3 CORNERS + CARDS V6 SPEED PASS")
