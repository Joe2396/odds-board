#!/usr/bin/env python3
# BETVICTOR_PLAYER_STATS_EXACT_FAST_TEST3_V8_NEAREST_SHOW_MORE
"""
...
Output:
    football/data/betvictor_player_stats_exact_fast_test_v8_nearest_show_more.json
...
OUT_PATH = DATA_DIR / "betvictor_player_stats_exact_fast_test_v8_nearest_show_more.json"
DEBUG_ROOT = ROOT / "football" / "debug" / "betvictor_player_stats_exact_fast_test_v8_nearest_show_more"
...
def mark_nearest_show_more_below_heading(
    ...
    """
    Mark the closest visible Show More below the exact requested heading.

    BetVictor places the green button just beneath the expanded five-row list,
    but it is not always inside the smallest DOM ancestor containing the rows.
    The candidate is accepted only when:
    - it follows the exact market heading in DOM order;
    - it is reasonably close below that heading;
    - the common ancestor contains at least one matching player row.
    """
    return page.evaluate(
        r"""({labels, patternSource}) => {
            document
            ...
            for (const element of rawShowMore) {
                const button =
                    element.closest(
                        "button, [role='button']"
                    )
            ...
            candidates.sort((left, right) => {
                ...
                return (
                    left.commonTextLength
                    - right.commonTextLength
                );
            });
...
def click_scoped_show_more_fast(
    ...
    """
    Expand the requested market using the nearest verified Show More below its
    exact heading. Re-locate after every DOM update.
    """
    clicks = 0
    passes = []
    pattern_source = config["row_re"].pattern
    ...
    for pass_number in range(
        ...
        # Refresh scope and collect everything currently visible.
        scope_before = mark_expanded_market_scope(
            page,
            config["headings"],
            pattern_source,
        )
        ...
        control = nearest_show_more_locator(page)
        ...
        # The expanded card may be replaced, so mark it again.
        scope_after = mark_expanded_market_scope(
            page,
            config["headings"],
            pattern_source,
        )
        ...
        # One short extra wait for slower list expansion.
        if after == before:
...
def main():
    ...
    print("BETVICTOR EXACT PLAYER STATS — FAST TEST3 V8 NEAREST SHOW MORE")
    print("=" * 72)
