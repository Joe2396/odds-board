#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_props_FAST_TEST3_V8_SCOPED_IMPACT_SUB.py

William Hill World Cup props scraper — FAST TEST3 V8 candidate — scoped Impact Sub player-stat parsing.
...
Output:
  football/data/williamhill_worldcup_props_FAST_TEST3_V8_SCOPED_IMPACT_SUB.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_TEST3_V8_SCOPED_IMPACT_SUB.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_TEST3_V8_SCOPED_IMPACT_SUB"
...
def scrape_match(page, fixture):
    ...
    try:
        ...
        print(f"    ✓ Popular/Default ({clicked} relevant headings clicked; {len(present)} found; ladders {dict(ladder_status)})")
    except Exception as exc:
    ...
    if player_tab:
        ...
        print(f"    ✓ {player_tab} ({clicked} relevant headings clicked; {len(present)} found; ladders {dict(ladder_status)})")
    else:
    ...
    if click_tab(page, "Goals"):
        ...
        print(f"    ✓ Goals ({clicked} relevant headings clicked; {len(present)} found; ladders {dict(ladder_status)})")
    else:
    ...
    if not player_tab:
        ...
        if all_tab:
            ...
            print(f"    ✓ {all_tab} fallback ({clicked} relevant headings clicked; {len(present)} found; ladders {dict(ladder_status)})")
    ...
    # Exact-heading diagnostic. These counts cannot bleed across Player Fouls
    # and Player Fouls Won and use only the currently visible Impact Sub rows.
    try:
        _, raw_stat_counts = parse_scoped_player_stat_markets(lines_from_text(all_text))
        print("    scoped Impact Sub rows: " + ", ".join(
        ...
        print(f"    scoped row diagnostic failed: {exc}")
    ...
    return {
        ...
        "player_price_mode": "impact_sub_default_visible",
...
def main():
    ...
    print("=" * 60)
    print("William Hill World Cup Props Scraper — FAST TEST3 V8 SCOPED IMPACT SUB")
