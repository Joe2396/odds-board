#!/usr/bin/env python3
"""
fetch_ladbrokes_worldcup_props_FAST_TEST3_V2_1_STABLE_PLAYER_FIX.py
...
OUT_PATH  = ROOT / "football" / "data" / "ladbrokes_worldcup_props_fast_test_v2_1_stable_player_fix.json"
DEBUG_DIR = ROOT / "football" / "debug" / "ladbrokes_worldcup_props_fast_test_v2_1_stable_player_fix"
...
def scrape_match(page, fixture):
    ...
    try:
        ...
        if marked.get("playerMarkets"):
            previous_player_text = card_text(
                page,
                "player-markets",
            )
            ...
            for (
                ...
                # Re-mark before every click because the previous filter may
                # have caused React to replace the card node.
                mark_player_stats_cards(page)
                ...
                player_text = refresh_player_cards_and_text(
                    page,
                    "player-markets",
                    previous_player_text,
                    timeout_ms=5000,
                )

                # The filter change can remount the card again when Show All
                # is clicked, so re-mark both before and after expansion.
                mark_player_stats_cards(page)
                ...
                print(
                    f"    player/{filter_name}: "
                    f"card_chars={len(player_text)} | "
                    f"lines={len(player_lines)} | "
                    f"changed="
                    f"{player_text != previous_player_text}"
                )
                ...
                previous_player_text = player_text
        else:
            print(
        ...
        if marked.get("tackles"):
            previous_tackle_text = card_text(
                page,
                "tackles",
            )
            ...
            for team in (home, away):
                ...
                tackle_text = refresh_player_cards_and_text(
                    page,
                    "tackles",
                    previous_tackle_text,
                    timeout_ms=5000,
                )
                ...
                tackle_lines = [
                    clean(line)
                    for line in tackle_text.splitlines()
                    if clean(line)
                ]

                team_market = parse_player_tackles(
                    tackle_lines
                )
                ...
                print(
                    ...
                    f"card_chars={len(tackle_text)} | "
                    f"lines={len(tackle_lines)} | "
                    f"changed="
                    f"{tackle_text != previous_tackle_text} | "
                    f"captured="
                    f"{team_market['selection_count']} | "
                    f"Show All clicks {expanded}"
                )

                previous_tackle_text = tackle_text
    ...
    missing_player_markets = sorted(
        player_expected - player_captured
    )

    print(
        "    player-market audit: "
        + (
            ", ".join(missing_player_markets)
            if missing_player_markets
            else "none"
        )
    )
    ...
    return {
        ...
        "missing_player_markets": missing_player_markets,
        "markets": unique,
    }
...
def main():
    ...
    print("=" * 60)
    print("Ladbrokes World Cup Props FAST TEST3 V2.1 STABLE PLAYER FIX")
    print("=" * 60)
