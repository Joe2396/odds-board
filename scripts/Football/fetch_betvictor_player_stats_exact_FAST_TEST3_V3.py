#!/usr/bin/env python3
# BETVICTOR_PLAYER_STATS_EXACT_FAST_TEST3_V3
"""
...
Output:
    football/data/betvictor_player_stats_exact_fast_test_v3.json
...
OUT_PATH = DATA_DIR / "betvictor_player_stats_exact_fast_test_v3.json"
DEBUG_ROOT = ROOT / "football" / "debug" / "betvictor_player_stats_exact_fast_test_v3"
...
NAVIGATION_SETTLE_MS = 1800
PLAYER_TAB_SETTLE_MS = 900
CLICK_SETTLE_MS = 650
HEADING_CLICK_RETRIES = 3
HEADING_SCAN_STEP = 500
HEADING_SCAN_WAIT_MS = 160
ROW_READY_TIMEOUT_MS = 4000
SCROLL_WAIT_MS = 170
SCROLL_END_WAIT_MS = 300
...
def wait_for_requested_rows(page, config, timeout_ms=ROW_READY_TIMEOUT_MS):
    pattern_source = config["row_re"].pattern
    ...
    try:
        ...
        return market_rows_visible(page, config)


def click_heading_with_javascript(page, labels):
    """Click the closest interactive ancestor of an exact heading label."""
    try:
        return page.evaluate(
            r"""(labels) => {
                ...
                const elements = Array.from(
                    document.querySelectorAll(
                        "button, [role='button'], summary, h1, h2, h3, h4, "
                        + "div, span, p"
                    )
                );

                for (const element of elements) {
                    const ownText = normalise(element.innerText);

                    if (!wanted.has(ownText)) {
                        continue;
                    }
...
def scan_and_click_market_heading(page, config):
    """
    Find and open a player-market accordion even when it is initially below the
    fold or appears shortly after the Player group loads.
    """
    labels = config["headings"]

    for attempt in range(1, HEADING_CLICK_RETRIES + 1):
        try:
        ...
        # The normal Playwright route is preferred.
        clicked = click_exact_text(page, labels)

        if clicked:
            if wait_for_requested_rows(page, config):
                return clicked

            # Some accordions need one additional direct click after animation.
            second_click = click_heading_with_javascript(page, labels)
            if second_click:
                page.wait_for_timeout(CLICK_SETTLE_MS)
        ...
        # Fallback: exact DOM-text click on the nearest interactive ancestor.
        clicked_js = click_heading_with_javascript(page, labels)
        ...
        # The heading may be lower down a lazy-rendered Player page.
        try:
            total_height = int(
                page.evaluate(
                    "Math.max("
        ...
        while position <= total_height:
            ...
            if not clicked:
                clicked = click_heading_with_javascript(page, labels)
            ...
            position += HEADING_SCAN_STEP

        # Reveal lazy sections before the next search pass.
        click_more_controls(page)
        page.wait_for_timeout(500)
...
def open_market_on_page(page, fixture, config, accept_once=False):
    ...
    for navigation_attempt in range(1, 3):
        ...
        if accept_once or navigation_attempt > 1:
            accept_cookies(page)

        # BetVictor often paints general odds before the Player accordions.
        page.wait_for_timeout(NAVIGATION_SETTLE_MS)

        player_clicked = click_exact_text(page, ["Player"])

        if player_clicked:
            page.wait_for_timeout(PLAYER_TAB_SETTLE_MS)

        # A direct player-group URL may already be selected, so absence of a
        # clickable Player tab is not itself a failure.
        if market_rows_visible(page, config):
            return {
                "heading_clicked": "already_open",
                "navigation_attempt": navigation_attempt,
                "player_tab_clicked": player_clicked,
                "rows_ready": True,
            }

        heading_clicked = scan_and_click_market_heading(
            page,
            config,
        )
        rows_ready = wait_for_requested_rows(
            page,
            config,
            timeout_ms=2500,
        )

        if heading_clicked or rows_ready:
            return {
        ...
        # One controlled reload is cheaper than returning an empty market.
        print(
            "    market heading not ready; "
            "retrying Player page once..."
        )

    return {
        "heading_clicked": None,
        "navigation_attempt": 2,
        "player_tab_clicked": None,
        "rows_ready": False,
    }
...
def scrape_market_on_page(
    ...
    navigation = open_market_on_page(
        page,
        fixture,
        config,
        accept_once=accept_once,
    )
    heading_clicked = navigation.get("heading_clicked")

    print(
        "    navigation: "
        f"attempt={navigation.get('navigation_attempt')} "
        f"player_tab={navigation.get('player_tab_clicked')} "
        f"heading={heading_clicked} "
        f"rows_ready={navigation.get('rows_ready')}"
    )
    ...
    for pass_number in range(1, MAX_HARVEST_PASSES + 1):
        ...
        # Always scan the full page. This gives us useful diagnostics even if
        # BetVictor changed the market heading text.
        add_rows(store, extractor())
    ...
    return {
        ...
        "heading_clicked": heading_clicked,
        "navigation_attempt": navigation.get("navigation_attempt"),
        "player_tab_clicked": navigation.get("player_tab_clicked"),
        "rows_ready_after_click": navigation.get("rows_ready"),
