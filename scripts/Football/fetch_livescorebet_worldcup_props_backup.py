...
PLAYER_GRP_ID  = "757"
MAX_MATCHES    = 15  # test first, then change to 15
...
def parse_scoped_ou(lines, heading, label_suffix, max_line=None, home=None, away=None):
    """
    Handles LiveScoreBet scoped tabs:
      Both Teams Combined / Home / Away
      Over / Under
      4.5
      1/10
      18/5

    Returns one market per scope found.
    """
...
def parse_scoped_markers(lines, heading, label_suffix, max_line=None):
    """Parse snapshots captured after clicking Both Teams Combined / home / away scope tabs."""
    markers = []
...
def parse_half_time_result(lines, home, away):
    ...
    # First try explicit "Half Time" heading.
    idx = next((i for i, l in enumerate(lines) if clean(l) in {"Half Time", "Half-Time Result", "Half Time Result"}), -1)

    # Fallback: LiveScoreBet sometimes only has "1st Half" rows under nearby match markets.
    if idx == -1:
...
def _parse_player_over_market(lines, header, market_name, prop_type):
    ...
    skip = {header, "View more"} | WORLD_CUP_TEAMS
    n = len(thresholds)
...
def parse_standard_props(text, home, away):
    ...
    # Scoped SOT and shots may return multiple markets: combined, home, away.
...
def click_market_heading_if_needed(page, heading):
    """Open a collapsed market accordion if needed."""
    try:
        ...
        # If odds already visible nearby, no need to click.
...
def click_scope_in_market(page, heading, scope):
    """Click Both Teams Combined / home / away inside one specific market section."""
    try:
        ok = page.evaluate(
            """({heading, scope}) => {
                const norm = s => (s || '').replace(/\s+/g, ' ').trim();
                const headingEls = Array.from(document.querySelectorAll('body *')).filter(e => norm(e.innerText) === heading);
                if (!headingEls.length) return false;
                ...
                for (const h of headingEls) {
                    ...
                    for (let d = 0; d < 9 && node; d++, node = node.parentElement) {
                        const txt = norm(node.innerText);
                        if (txt.includes(heading) && txt.includes('Both Teams Combined') && txt.includes(scope)) {
                            container = node;
                            break;
                        }
                    }
                ...
                const candidates = Array.from(container.querySelectorAll('*')).filter(e => norm(e.innerText) === scope);
...
def collect_shots_scope_text(page, home, away):
    """Capture body text after clicking each scope for Total Shots and Total Shots on Target."""
    chunks = []
...
def scrape_match(page, fixture):
    ...
    # Safe extra pass: keep the original standard text, then append scope snapshots.
    # If clicks fail, the original 16/17-market scrape still works.
