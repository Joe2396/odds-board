#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_props_FAST_TEST3_V13_BOTH_SCORER_TEAMS.py

William Hill World Cup props scraper — FAST TEST3 V13 candidate — both scorer team panels.
...
Output:
  football/data/williamhill_worldcup_props_FAST_TEST3_V13_BOTH_SCORER_TEAMS.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_TEST3_V13_BOTH_SCORER_TEAMS.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_TEST3_V13_BOTH_SCORER_TEAMS"
...
def v13_click_scorer_team_panel(page, team, before_text=""):
    """Click the large team selector inside Scorer Markets, not the event header.

    William Hill repeats team names in several places.  The old generic clicker
    could hit the fixture header instead of the scorer-team box.  This version
    scopes candidates to the narrow vertical band immediately below the exact
    `Scorer Markets` heading and then verifies that the player-grid text changed.
    """
    js = r"""
        (team) => {
            const norm = s => (s || '').replace(/\s+/g, ' ').trim().toLowerCase();
            ...
            const headingNodes = Array.from(document.querySelectorAll(
                'h1,h2,h3,h4,div,span,button,[role=button]'
            )).filter(el => visible(el) && norm(el.innerText || el.textContent || '') === 'scorer markets');

            if (!headingNodes.length) return {status:'heading_not_found'};

            headingNodes.sort((a,b) => {
            ...
            for (const el of exact) {
                const er = el.getBoundingClientRect();
                if (er.top < hr.bottom - 12 || er.top > hr.bottom + 190) continue;

                let click = el;
                let cur = el;
                for (let depth = 0; depth < 8 && cur && cur !== document.body; depth++, cur = cur.parentElement) {
                    if (!visible(cur)) continue;
                    const r = cur.getBoundingClientRect();
                    const txt = norm(cur.innerText || cur.textContent || '');
                    if (!txt.includes(target)) continue;
                    if (r.width >= 220 && r.width <= 1100 && r.height >= 32 && r.height <= 115 &&
                        r.top >= hr.bottom - 15 && r.top <= hr.bottom + 190) {
                        click = cur;
                        break;
                    }
                }
                ...
                const r = click.getBoundingClientRect();
                let score = 0;
                score += Math.min(r.width, 1000);
                score -= Math.abs(r.top - (hr.bottom + 55)) * 5;
                if (click.matches('button,[role=button],[role=tab],a')) score += 300;
                if (click.getAttribute('aria-selected') === 'true') score -= 1000;
                candidates.push({
                    x: r.left + r.width / 2,
                    y: r.top + r.height / 2,
                    left: r.left,
                    ...
                    score,
                    tag: click.tagName,
                    role: click.getAttribute('role') || '',
                    ariaSelected: click.getAttribute('aria-selected') || '',
                    cls: String(click.className || '').slice(0, 300)
                });
            }

            candidates.sort((a,b) => b.score - a.score);
            return candidates.length ? {status:'candidate', candidate:candidates[0], count:candidates.length} : {status:'team_panel_not_found'};
        }
    """

    try:
        # Keep the scorer team boxes visible before locating them.
        set_accordion_open(page, ["Scorer Markets"], True)
        scroll_heading_into_view(page, "Scorer Markets")
        page.wait_for_timeout(300)
        located = page.evaluate(js, team)
        if not located or located.get("status") != "candidate":
            return False, {"status": (located or {}).get("status", "no_candidate")}

        point = located["candidate"]
        page.mouse.click(float(point["x"]), float(point["y"]))
        ...
        for _ in range(16):
            page.wait_for_timeout(250)
            after_text = v13_visible_scorer_block_text(page)
            if before_text and after_text and after_text != before_text:
                changed = True
                break

        # A selected home panel may already be active, so unchanged text is not
        # automatically a failure.  For the away panel we require a real grid
        # change before accepting it.
        return changed, {
            "status": "clicked_grid_changed" if changed else "clicked_but_grid_unchanged",
            "candidate_count": located.get("count", 0),
            "candidate": point,
        }
    except Exception as exc:
        return False, {"status": "error", "error": f"{type(exc).__name__}: {exc}"}
...
def v12_capture_scorers_on_impact_tab(page, home, away):
    """Capture First, Anytime and 2+ from both scorer-team panels.

    The left/home team is William Hill's default visible panel.  Parse it first,
    then click the large right/away team box scoped beneath `Scorer Markets` and
    require the player grid to change before parsing the away side.
    """
    ...
    # William Hill defaults to the left/home team in this grid.
    ...
    # Click only the large away-team selector inside Scorer Markets.
    away_changed, away_click_diag = v13_click_scorer_team_panel(page, away, home_text)
    away_parsed = []
    away_text = ""
    if away_changed:
        click_all_see_more(page, max_clicks=8)
    ...
    away_counts = {m["normalized_market"]: m["selection_count"] for m in away_parsed}
    team_results.append({
        "team": away,
        "panel": "right",
        "clicked": away_changed,
        "click_diagnostics": away_click_diag,
        "counts": away_counts,
    })
    dumps.append(
        f"=== IMPACT SUB SCORER TEAM {away} changed={away_changed} "
        f"diag={away_click_diag} ===\n{away_text}"
    )
...
def scrape_match(page, fixture):
    ...
    return {
        ...
        "url": url,
        "url_source": fixture.get("url_source", ""),
        "captured_tabs": ["Popular/Default", "Impact Sub"],
        "player_price_mode": "top_level_impact_sub_tab",
        "capture_strategy": "v13_top_impact_sub_both_scorer_teams",
        "diagnostics": str(diag_path),
        "market_count": len(markets),
        "player_market_count": player_market_count,
        "player_selection_count": player_selection_count,
...
def main():
    ...
    print("=" * 60)
    print("William Hill World Cup Props Scraper — FAST TEST3 V13 BOTH SCORER TEAM PANELS")
