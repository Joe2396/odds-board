#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_props_FAST_TEST3_V9_DOM_GEOMETRY.py

William Hill World Cup props scraper — FAST TEST3 V9 candidate — DOM geometry player-stat parsing.
...
Output:
  football/data/williamhill_worldcup_props_FAST_TEST3_V9_DOM_GEOMETRY.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_TEST3_V9_DOM_GEOMETRY.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_TEST3_V9_DOM_GEOMETRY"
...
def scrape_match(page, fixture):
    ...
    chunks = []
    captured_tabs = []
    dom_stat_rows = {key: [] for key in DOM_STAT_TARGETS}
    dom_diagnostics = {}

    # Default/Popular contains core match markets and often some player markets.
    try:
        clicked, present, ladder_status = expand_relevant_player_markets_fast(page)
        scroll_page(page, 5)
        dom_rows, dom_diag = extract_dom_player_stat_rows(page, "Popular/Default")
        merge_dom_row_buckets(dom_stat_rows, dom_rows)
        dom_diagnostics["Popular/Default"] = dom_diag
        chunks.append("=== TAB Popular/Default ===\n" + page.locator("body").inner_text(timeout=18000))
        captured_tabs.append("Popular/Default")
        print(f"    ✓ Popular/Default ({clicked} relevant headings clicked; {len(present)} found; DOM rows " +
              str({MARKET_META[k][0]: len(dom_rows.get(k, [])) for k in DOM_STAT_TARGETS}) + ")")
    except Exception as exc:
    ...
    if player_tab:
        clicked, present, ladder_status = expand_relevant_player_markets_fast(page)
        scroll_page(page, 7)
        dom_rows, dom_diag = extract_dom_player_stat_rows(page, player_tab)
        merge_dom_row_buckets(dom_stat_rows, dom_rows)
        dom_diagnostics[player_tab] = dom_diag
        chunks.append(f"=== TAB {player_tab} ===\n" + page.locator("body").inner_text(timeout=20000))
        captured_tabs.append(player_tab)
        print(f"    ✓ {player_tab} ({clicked} relevant headings clicked; {len(present)} found; DOM rows " +
              str({MARKET_META[k][0]: len(dom_rows.get(k, [])) for k in DOM_STAT_TARGETS}) + ")")
    else:
    ...
    if click_tab(page, "Goals"):
        clicked, present, ladder_status = expand_relevant_player_markets_fast(page)
        scroll_page(page, 6)
        dom_rows, dom_diag = extract_dom_player_stat_rows(page, "Goals")
        merge_dom_row_buckets(dom_stat_rows, dom_rows)
        dom_diagnostics["Goals"] = dom_diag
        chunks.append("=== TAB Goals ===\n" + page.locator("body").inner_text(timeout=20000))
        captured_tabs.append("Goals")
        print(f"    ✓ Goals ({clicked} relevant headings clicked; {len(present)} found; DOM rows " +
              str({MARKET_META[k][0]: len(dom_rows.get(k, [])) for k in DOM_STAT_TARGETS}) + ")")
    else:
    ...
    if not player_tab:
        ...
        if all_tab:
            clicked, present, ladder_status = expand_relevant_player_markets_fast(page)
            scroll_page(page, 7)
            dom_rows, dom_diag = extract_dom_player_stat_rows(page, all_tab)
            merge_dom_row_buckets(dom_stat_rows, dom_rows)
            dom_diagnostics[all_tab] = dom_diag
            chunks.append(f"=== TAB {all_tab} ===\n" + page.locator("body").inner_text(timeout=20000))
            captured_tabs.append(all_tab)
            print(f"    ✓ {all_tab} fallback ({clicked} relevant headings clicked; {len(present)} found; DOM rows " +
                  str({MARKET_META[k][0]: len(dom_rows.get(k, [])) for k in DOM_STAT_TARGETS}) + ")")
    ...
    # V9: the five count-ladder player markets are replaced by DOM-geometry
    # extraction. Text fallbacks are deliberately not trusted for these markets.
    dom_markets, dom_counts = build_dom_player_stat_markets(dom_stat_rows)
    markets = replace_count_ladder_markets(markets, dom_markets)

    dom_debug_path = DEBUG_DIR / f"{slugify(name)}_dom_rows.json"
    dom_debug_path.write_text(json.dumps({
        "rows": dom_stat_rows,
        "diagnostics": dom_diagnostics,
        "parsed_counts": dom_counts,
    }, indent=2, ensure_ascii=False), encoding="utf-8")

    print("    DOM geometry parsed: " + ", ".join(
        f"{MARKET_META[key][0]}={dom_counts.get(key, 0)}"
        for key in DOM_STAT_TARGETS
    ))
    ...
    if identical_market_fingerprint(markets, "Player Fouls Committed", "Player Fouls Won"):
        validation_warnings.append(
            "Player Fouls Committed and Player Fouls Won are still identical; DOM sectioning failed"
        )
    ...
    return {
        ...
        "player_price_mode": "impact_sub_default_visible_no_mode_clicks",
        "dom_geometry_debug": str(dom_debug_path),
...
def main():
    ...
    print("=" * 60)
    print("William Hill World Cup Props Scraper — FAST TEST3 V9 DOM GEOMETRY")
