#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_props_FAST_TEST3_V12_IMPACT_SUB_TAB.py

William Hill World Cup props scraper — FAST TEST3 V12 candidate — top-level Impact Sub tab.
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_TEST3_V12_IMPACT_SUB_TAB.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_TEST3_V12_IMPACT_SUB_TAB"
...
def v12_capture_scorers_on_impact_tab(page, home, away):
    """Capture First, Anytime and 2+ from both team panels on Impact Sub."""
    ...
    for team in [home, away]:
        # Re-scroll the scorer accordion before each team click because a prior
        # Show More can move the grid outside the viewport.
        set_accordion_open(page, ["Scorer Markets"], True)
        clicked = click_scorer_team(page, team)
        ...
        parse_text = block_text or full_text

        parsed = v11_parse_flexible_scorer_grid(parse_text)
        if not parsed:
            parsed = parse_scorer_grid_three_markets(parse_text)

        counts = {m["normalized_market"]: m["selection_count"] for m in parsed}
        team_results.append({"team": team, "clicked": clicked, "counts": counts})
        dumps.append(f"=== IMPACT SUB SCORER TEAM {team} clicked={clicked} ===\n{parse_text}")
        for market in parsed:
            key = normalize(market.get("normalized_market") or market.get("market"))
            if key in combined:
                combined[key].extend(market.get("selections", []))
...
def scrape_match(page, fixture):
    ...
    return {
        ...
        "capture_strategy": "v12_top_impact_sub_exact_whitelist",
        "diagnostics": str(diag_path),
        "market_count": len(markets),
        "player_market_count": player_market_count,
        "player_selection_count": player_selection_count,
...
def main():
    ...
    print("=" * 60)
    print("William Hill World Cup Props Scraper — FAST TEST3 V12 TOP IMPACT SUB TAB")
