#!/usr/bin/env python3
"""
run_williamhill_props_FAST_PROD15_V21_AVAILABILITY_ONE_FILE.py

William Hill World Cup props scraper — FAST PROD15 V21 availability candidate — respect unpublished Impact Sub tabs.
...
Output:
  football/data/williamhill_worldcup_props_FAST_PROD15_V21_AVAILABILITY_CANDIDATE.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_PROD15_V21_AVAILABILITY_CANDIDATE.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_PROD15_V21_AVAILABILITY_CANDIDATE"
...
def scrape_match(page, fixture):
    ...
    if not tab_ok:
        ...
        return {
            ...
            "capture_strategy": "v21_availability_aware_repair",
            "diagnostics": str(diag_path),
            "market_count": len(markets),
...
def _load_v19_output():
    if not PREVIOUS_V19_PATH.exists():
        raise RuntimeError(
            "Missing V19 candidate JSON. Run the V19 resume candidate first:\n"
            f"{PREVIOUS_V19_PATH}"
        )

    data = json.loads(PREVIOUS_V19_PATH.read_text(encoding="utf-8"))
    rows = data.get("matches", []) if isinstance(data, dict) else []
    if not isinstance(rows, list):
        raise RuntimeError("V19 candidate JSON has no matches list")
    return data, rows
...
def main():
    ...
    print("=" * 72)
    print("William Hill FAST PROD15 V21 — AVAILABILITY-AWARE REPAIR")
    print("Fixtures without a published Impact Sub tab are accepted as Popular-only")
    print("Production williamhill_worldcup_props.json will NOT be changed")
    print("=" * 72)

    previous_data, previous_rows = _load_v19_output()
    if len(previous_rows) != 15:
        raise RuntimeError(
            f"Expected 15 V19 fixture records, found {len(previous_rows)}"
        )
    ...
    print(f"Fixtures needing repair: {len(repair_indexes)}")
    ...
    output = {
        ...
        "elapsed_seconds": elapsed,
        "repair_source": str(PREVIOUS_V19_PATH),
        "capture_strategy": "v21_availability_aware_repair",
        "matches": results,
    }
    ...
    print()
    print("=" * 72)
    print("Running availability-aware V21 validation")
    print("=" * 72)
