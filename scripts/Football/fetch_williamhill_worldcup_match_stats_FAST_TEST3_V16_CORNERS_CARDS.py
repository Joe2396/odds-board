#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_match_stats_FAST_TEST3_V16_CORNERS_CARDS.py

FAST TEST3 V16: V23 URL cache plus exact scoped Corners/Cards parsing.
...
OUT_PATH = ROOT / "football" / "data" / "williamhill_worldcup_match_stats_FAST_TEST3_V16_CORNERS_CARDS.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_match_stats_FAST_TEST3_V16_CORNERS_CARDS"
...
def scrape_tab_market(page, tab_name, heading_candidates, market_name, prop_type, debug_chunks):
    """V16: one exact heading, scoped Match tab, direct outcome-button parser."""
    ...
    heading, open_diag = v16_open_exact_market(
        page,
        heading_candidates,
    )

    if not heading:
        print(f"      - no exact {tab_name} total heading published")
        debug_chunks.append(
            f"\n\n=== TAB {tab_name} / NO EXACT HEADING ===\n"
            + json.dumps(open_diag, indent=2)
        )
        return None

    print(f"      clicked exact {heading}")

    match_clicked = v16_click_inner_match_scoped(page, heading)
    if match_clicked:
        print(f"        clicked scoped inner Match tab")

    market, parse_diag = v16_extract_scoped_ou_buttons(
        page,
        heading,
        market_name,
        prop_type,
    ...
    try:
        section_text = page.locator("body").inner_text(timeout=12000)
    except Exception:
        section_text = ""
...
def main():
    ...
    print("=" * 60)
    print("William Hill Match/Team Stats — FAST TEST3 V16 CORNERS + CARDS")
