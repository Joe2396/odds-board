#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_props_PRODUCTION_V23.py
...
Output:
  football/data/williamhill_worldcup_props_V23_STAGING.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_V23_STAGING.json"
LIVE_OUT_PATH = ROOT / "football" / "data" / "williamhill_worldcup_props.json"
BACKUP_DIR = ROOT / "football" / "data" / "backups"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_V23_STAGING"
...
def scrape_match(page, fixture):
    ...
    if not tab_ok:
        ...
        return {
            ...
            "capture_strategy": "v23_production_validated_full_run",
            "diagnostics": str(diag_path),
            "market_count": len(markets),
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            fixtures = get_match_links(discovery_page)
        finally:
        ...
        for index, fixture in enumerate(fixtures, start=1):
            print()
            print("=" * 72)
            print(f"[{index}/{len(fixtures)}] {fixture['name']}")

            result = _scrape_with_retries(
                browser,
                fixture,
                max_attempts=2,
            )
    ...
    output = {
        ...
        "player_price_mode": "top_level_impact_sub_tab_when_available",
    ...
    # Always save the full run to staging first. Production remains untouched
    # until every validation check passes.
    staging_temp = OUT_PATH.with_suffix(OUT_PATH.suffix + ".tmp")
    staging_temp.write_text(
        json.dumps(output, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    staging_temp.replace(OUT_PATH)

    print()
    print("── V23 production summary ─────────────────────────────────────")
    for row in results:
    ...
    (
        ...
        impact_available_count,
        popular_only_count,
    ) = _validate_v23(output)

    print(
        f"Impact Sub availability: {impact_available_count}/"
    ...
    for key, count in sorted(optional_coverage.items()):
        print(f"  {key:<30} {count:>2}/{len(results)}")

    report = {
        "status": "PASS" if not failures else "FAIL",
        "validated_at": datetime.now(timezone.utc).isoformat(),
        "staging_input": str(OUT_PATH),
        "live_output": str(LIVE_OUT_PATH),
    ...
    report_path = DEBUG_DIR / "production_validation_report.json"
    ...
    if failures:
        ...
        print("Live production JSON was NOT changed.")
        return 1

    # Back up the previous live output before atomic promotion.
    backup_path = None
    if LIVE_OUT_PATH.exists():
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        backup_path = (
            BACKUP_DIR
            / f"williamhill_worldcup_props_before_v23_{stamp}.json"
        )
        backup_path.write_bytes(LIVE_OUT_PATH.read_bytes())

    live_temp = LIVE_OUT_PATH.with_suffix(LIVE_OUT_PATH.suffix + ".tmp")
    live_temp.write_bytes(OUT_PATH.read_bytes())
    live_temp.replace(LIVE_OUT_PATH)

    print()
    print("VALIDATION PASS")
    print("Live production JSON updated atomically.")
    print(f"Live output: {LIVE_OUT_PATH}")
    if backup_path:
