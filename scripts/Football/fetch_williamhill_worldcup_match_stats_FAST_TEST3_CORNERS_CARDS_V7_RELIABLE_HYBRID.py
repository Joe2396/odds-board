#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_match_stats_FAST_TEST3_CORNERS_CARDS_V7_RELIABLE_HYBRID.py

FAST TEST3 match-stats V7: reliable V4 base with direct full-ladder polling.
...
OUT_PATH = ROOT / "football" / "data" / "williamhill_worldcup_match_stats_FAST_TEST3_CORNERS_CARDS_V7_RELIABLE_HYBRID.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_match_stats_FAST_TEST3_CORNERS_CARDS_V7_RELIABLE_HYBRID"
...
def scrape_tab_market(page, tab_name, heading_candidates, market_name, prop_type, debug_chunks):
    """V7 reliable hybrid: direct heading, patient polling, no alias loops."""
    ...
    target_count = 10 if prop_type == "corners" else 12
    ...
    print(
        ...
        + (" (retry)" if tab_retry else "")
    )

    if not clicked_tab:
        debug_chunks.append(
            f"\n\n=== V7 TAB {tab_name} ABSENT ===\n"
            + json.dumps(
                {
    ...
    if not opened_heading:
        ...
        debug_chunks.append(
            f"\n\n=== V7 TAB {tab_name} HEADING ABSENT ===\n"
            + json.dumps(
                {
                    "preferred_heading": preferred_heading,
                    "open": open_diag,
    ...
    # V4 proved the complete ladder arrives without Show More, but only after
    # William Hill finishes lazy rendering. Poll the same market patiently.
    for wait_ms in [700, 900, 1200, 1600, 2200, 3000, 3800]:
        ...
        if best_market["selection_count"] >= target_count:
            break

    # One final reliable Playwright body read if the fast snapshots were still
    # partial. This keeps V4's proven parser behaviour as the fallback.
    final_fallback_used = False
    if best_market["selection_count"] < target_count:
        final_fallback_used = True
        try:
            body_text = page.locator("body").inner_text(timeout=18000)
        except Exception:
            body_text = ""

        fallback_market = parse_tab_ou_market_from_text(
            body_text,
            [opened_heading],
            market_name,
            prop_type,
        ...
        if (
            fallback_market["selection_count"] >
            best_market["selection_count"]
        ):
    ...
    print(
        ...
        f"[{quality}] in {elapsed}s"
    )

    debug_chunks.append(
        f"\n\n=== MATCH-STATS V7 TAB {tab_name} ===\n"
        + json.dumps(
            {
                "preferred_heading": preferred_heading,
                "target_count": target_count,
...
def scrape_match_stats(page, fixture):
    ...
    for key, minimum in expected_counts.items():
        ...
        actual = market["selection_count"] if market else 0
        if actual < minimum:
            quality_failures.append(
                f"{key}: expected at least {minimum}, got {actual}"
            )
    ...
    if quality_failures:
        ...
        print("    QUALITY PASS: all 8 markets are complete")
...
def main():
    ...
    print("=" * 60)
    print("William Hill Match/Team Stats — FAST TEST3 CORNERS + CARDS V7 RELIABLE HYBRID")
