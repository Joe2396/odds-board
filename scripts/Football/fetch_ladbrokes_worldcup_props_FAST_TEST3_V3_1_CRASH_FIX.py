#!/usr/bin/env python3
"""
fetch_ladbrokes_worldcup_props_FAST_TEST3_V3_1_CRASH_FIX.py
...
OUT_PATH  = ROOT / "football" / "data" / "ladbrokes_worldcup_props_fast_test_v3_1_crash_fix.json"
DEBUG_DIR = ROOT / "football" / "debug" / "ladbrokes_worldcup_props_fast_test_v3_1_crash_fix"
...
def teams_from_fixture_url(url, fallback_name=""):
    """
    Derive teams before page scraping so home/away are available to every
    navigation stage, including scoped corners/cards extraction.
    """
    slug_part = (
        clean(url)
...
def scrape_match(page, fixture):
    ...
    print(
        "    Double Chance: using expanded main-page text "
        "with all-occurrence parser"
    )
    ...
    detected_home, detected_away = detect_teams(
        all_text
    )

    if detected_home and detected_away:
        home = detected_home
        away = detected_away
...
def main():
    ...
    print("=" * 60)
    print("Ladbrokes World Cup Props FAST TEST3 V3.1 CRASH FIX")
    print("=" * 60)
