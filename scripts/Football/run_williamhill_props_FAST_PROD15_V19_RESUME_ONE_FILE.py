#!/usr/bin/env python3
"""
run_williamhill_props_FAST_PROD15_V19_RESUME_ONE_FILE.py

William Hill World Cup props scraper — FAST PROD15 V19 resume candidate — robust Impact Sub locator.
...
Output:
  football/data/williamhill_worldcup_props_FAST_PROD15_V19_RESUME_CANDIDATE.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_PROD15_V19_RESUME_CANDIDATE.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_PROD15_V19_RESUME_CANDIDATE"
...
def scrape_match(page, fixture):
    ...
    return {
        ...
        "capture_strategy": "v19_resume_robust_top_impact_sub_locator",
        "diagnostics": str(diag_path),
        "market_count": len(markets),
        "player_market_count": player_market_count,
        "player_selection_count": player_selection_count,
...
def _load_previous_v18_results():
    if not PREVIOUS_V18_PATH.exists():
        return {}
    try:
        data = json.loads(PREVIOUS_V18_PATH.read_text(encoding="utf-8"))
    except Exception:
        return {}
...
def _is_successful_existing_result(row):
    return bool(
        isinstance(row, dict) and
        row.get("market_count", 0) > 0 and
        not clean(row.get("error"))
    )


def _validate_v19_output(output):
    failures = []
    ...
    coverage = {key: 0 for key in target_keys}
    ...
    for fixture in fixtures:
        ...
        present = target_keys & set(markets)
        for key in present:
            coverage[key] += 1

        if len(present) < 8:
            failures.append(
                f"{name}: only {len(present)}/10 target markets present"
            )
        ...
        for key in {"first_goalscorer", "anytime_scorer", "scorer_2_plus"}:
            ...
            if not market:
                continue
            panels = {
                clean(selection.get("source_team_panel")).lower()
                for selection in market.get("selections", [])
                if clean(selection.get("source_team_panel"))
            }
            if len(panels) < 2:
                failures.append(
                    f"{name}: {key} contains only {len(panels)} scorer panel(s)"
                )

        for key in {"player_shots", "player_tackles"}:
            market = markets.get(key)
            if market and len(market.get("selections", [])) <= 10:
                failures.append(
        ...
        for market in markets.values():
            ...
            for selection in rows:
                ...
                prop = normalize(selection.get("prop_type"))

                duplicate_key = (
                    player.lower(), threshold.lower(),
                    line.lower(), price, prop
                )
                if duplicate_key in seen:
                    failures.append(
                        f"{name}: exact duplicate in {market.get('market')}: "
                        f"{player} {threshold} {price}"
                    )
                    break
                ...
                if match:
                    ...
                    try:
                        ...
                        failures.append(
                            f"{name}: invalid line for {player} {threshold}"
                        )
                        break
                    if abs(expected - actual) > 1e-9:
                        failures.append(
                            f"{name}: {player} {threshold} expected line "
        ...
        fouls_committed = fingerprint("player_fouls_committed")
        fouls_won = fingerprint("player_fouls_won")
        if fouls_committed and fouls_committed == fouls_won:
            failures.append(
                f"{name}: Fouls Committed and Fouls Won are exact copies"
            )

        shots = fingerprint("player_shots")
        sot = fingerprint("player_shots_on_target")
        if shots and shots == sot:
            failures.append(
    ...
    for key, count in sorted(coverage.items()):
        if count < required_coverage:
            failures.append(
                f"Coverage collapse: {key} appears on {count}/15 fixtures; "
                f"minimum is {required_coverage}/15"
            )
...
def main():
    ...
    print("=" * 72)
    print("William Hill FAST PROD15 V19 — RESUME FAILED FIXTURES")
    print("Robust top-level Impact Sub locator")
    print("Production williamhill_worldcup_props.json will NOT be changed")
    print("=" * 72)

    run_started = time.perf_counter()
    previous = _load_previous_v18_results()
    ...
    with sync_playwright() as p:
        ...
        for index, fixture in enumerate(fixtures, start=1):
            key = normalize(fixture["name"])
            existing = previous.get(key)

            print("\n" + "=" * 72)
            print(f"[{index}/{len(fixtures)}] {fixture['name']}")

            if _is_successful_existing_result(existing):
                print(
                    "  ✓ Preserving successful V18 result — no browser rerun "
                    f"({existing.get('market_count', 0)} markets, "
                    f"{existing.get('player_selection_count', 0)} player selections)"
                )
                results.append(existing)
                continue

            print("  Retrying failed fixture with robust Impact Sub locator")
            try:
    ...
    output = {
        ...
        "matches_with_markets": len(
            [row for row in results if row.get("market_count", 0) > 0]
        ),
        ...
        "elapsed_seconds": elapsed,
        "resume_source": str(PREVIOUS_V18_PATH),
        "capture_strategy": "v19_resume_robust_top_impact_sub_locator",
        "matches": results,
    }
    ...
    print("── Summary ─────────────────────────────────────────────────────")
    for row in results:
        status = "OK" if row.get("market_count", 0) > 0 else "FAIL"
        print(
            f"  [{status:<4}] {row.get('match', ''):<38} "
            f"{row.get('market_count', 0):>2} markets | "
            f"{row.get('player_selection_count', 0):>4} player selections | "
            f"{row.get('elapsed_seconds', 0):>6}s"
        )
    print(f"Resume runtime: {elapsed}s")
    print(f"Saved candidate: {OUT_PATH}")

    print()
    print("=" * 72)
    print("Running integrated V19 validation")
    print("=" * 72)

    failures, coverage = _validate_v19_output(output)

    print("Target-market coverage:")
    for key, count in sorted(coverage.items()):
    ...
    print("V19 candidate is ready for local review.")
    print("Existing production remains untouched.")
    return 0
