#!/usr/bin/env python3
# BETVICTOR_PLAYER_STATS_EXACT_FAST_TEST3_V4_SAFE
"""
...
Output:
    football/data/betvictor_player_stats_exact_fast_test_v4_safe.json
...
OUT_PATH = DATA_DIR / "betvictor_player_stats_exact_fast_test_v4_safe.json"
DEBUG_ROOT = ROOT / "football" / "debug" / "betvictor_player_stats_exact_fast_test_v4_safe"
...
def scrape_market(browser, fixture, key, config):
    ...
    try:
        ...
        initial_added = add_rows(store, extractor())
        phase_counts["initial_rows"] = len(store)

        more_clicked = click_more_controls(page)
        phase_counts["first_show_more_clicked"] = more_clicked
        phase_counts["after_first_show_more"] = len(store)

        harvest_window(page, extractor, store)
        phase_counts["after_first_window"] = len(store)

        containers = harvest_containers(page, extractor, store)
        phase_counts["containers_found_first"] = len(containers)
        phase_counts["after_first_containers"] = len(store)

        more_clicked_second = click_more_controls(page)
        more_clicked += more_clicked_second
        phase_counts["second_show_more_clicked"] = more_clicked_second

        add_rows(store, extractor())
        phase_counts["before_second_window"] = len(store)

        harvest_window(page, extractor, store)
        phase_counts["after_second_window"] = len(store)
        ...
        phase_counts["containers_found_second"] = len(
            containers_second
        )
        phase_counts["after_second_containers"] = len(store)

        print(
            "    harvest phases: "
            f"initial={phase_counts['initial_rows']} "
            f"show_more={more_clicked} "
            f"containers="
            f"{phase_counts['containers_found_first']}/"
            f"{phase_counts['containers_found_second']} "
...
def main():
    ...
    print("BETVICTOR EXACT PLAYER STATS — FAST TEST3 V4 SAFE")
    print("=" * 72)
