...
OUT_PATH  = ROOT / "football" / "data" / "livescorebet_worldcup_props_FAST_TEST3_V2_SAFE_WAITS.json"
DEBUG_DIR = ROOT / "football" / "debug" / "livescorebet_worldcup_props_FAST_TEST3_V2_SAFE_WAITS"
...
def fast_body_text(page):
    """Fast DOM text snapshot, with Playwright fallback if the page is sparse."""
    try:
        text = page.evaluate(
...
def wait_for_market_container(
    page,
    heading,
    require_prices=True,
    timeout_ms=1200,
):
    """Poll for the market container instead of always sleeping the full wait."""
    started = time.perf_counter()
    last_container = None

    while (time.perf_counter() - started) * 1000 < timeout_ms:
        container = _market_container(
            page,
            heading,
        ...
        if not require_prices:
            last_container = _market_container(
                page,
                heading,
    ...
    return last_container, round(
        time.perf_counter() - started,
        2,
    )


def wait_for_scope_ready(
    page,
    heading,
    scope,
    all_scopes,
    ...
    """
    Wait for prices and, where exposed, the requested active tab.

    This preserves the V1 fail-closed behaviour. It only returns early when
    the same evidence V1 eventually checked is already available.
    """
    started = time.perf_counter()
    best_container = None
    best_active = ""
    ...
    while (time.perf_counter() - started) * 1000 < timeout_ms:
        ...
        if container is not None:
            ...
            active_scope = _detect_active_scope(
                container,
                all_scopes,
            )
            best_active = active_scope

            # Explicitly correct active scope: ready immediately.
            ...
            # No explicit active-state markup. V1 accepted this as long as the
            # scoped card contained prices; wait a small minimum for rerender.
            if not active_scope:
                elapsed_ms = (
                    time.perf_counter() - started
                ) * 1000
...
def click_market_heading_if_needed(page, heading):
    try:
        ...
        if container is None:
            ...
            container, _ = wait_for_market_container(
                page,
                heading,
                require_prices=False,
                timeout_ms=1000,
...
def capture_scope_market_text(page, heading, scope, all_scopes):
    ...
    V2 preserves V1's active-scope verification, but proceeds as soon as the
    requested scope and prices are ready rather than sleeping a fixed 1.4s.
    """
    container = _market_container(
        page,
        heading,
    ...
    try:
        ...
        print(
            f"    scope click failed: "
            f"{heading} / {scope}: {e}"
        )
        return ""

    container, active_scope, ready_seconds = wait_for_scope_ready(
        page,
        heading,
        scope,
        all_scopes,
    ...
    if active_scope and active_scope != scope:
        # One retry, same as V1, but readiness is polled rather than fixed.
        ...
        if retry_item is not None:
            try:
                ...
                ready_seconds += retry_seconds

                if retry_container is not None:
                    container = retry_container
                active_scope = retry_active
            except Exception:
                pass
    ...
    print(
        f"      scope ready in {ready_seconds:.2f}s"
    )
    return text
...
def scrape_match(page, fixture):
    ...
    market_counts = {
        market["normalized_market"]: market["selection_count"]
    ...
    missing_required = sorted(
        required_markets - set(market_counts)
    )
    quality_status = (
        "PASS"
...
def main():
    ...
    print("=" * 60)
    print("LiveScoreBet Props — FAST TEST3 V2 SAFE WAITS")
    print("=" * 60)
    ...
    output = {
        ...
        "market_type": "props",
        "scraper_version": "fast_test3_v2_safe_waits",
