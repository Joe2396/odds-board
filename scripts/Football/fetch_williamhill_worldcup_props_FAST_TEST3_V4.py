#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_props_FAST_TEST3_V4.py

William Hill World Cup props scraper — FAST TEST3 V4 candidate — canonical William Hill shot thresholds.
...
Output:
  football/data/williamhill_worldcup_props_FAST_TEST3_V4.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_TEST3_V4.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_TEST3_V4"
...
INLINE_SHOT_ROW_PATTERNS = [
    re.compile(
        rf"^(?P<player>.+?)\s+At\s+Least\s+(?P<count>{COUNT_TOKEN_PATTERN})\s+"
        rf"(?P<stat>Shots?(?:\s+On\s+Target)?)$",
        re.I,
    ),
    re.compile(
        rf"^(?P<player>.+?)\s+Over\s+(?P<count>{COUNT_TOKEN_PATTERN})\s+"
        rf"(?P<stat>Shots?(?:\s+On\s+Target)?)$",
        re.I,
    ),
    re.compile(
        rf"^(?P<player>.+?)\s+(?P<count>\d+)\+\s+"
        rf"(?P<stat>Shots?(?:\s+On\s+Target)?)$",
        re.I,
    ),
    re.compile(
        rf"^(?P<player>.+?)\s+(?:To\s+Have\s+)?(?P<count>{COUNT_TOKEN_PATTERN})\s+"
        rf"Or\s+More\s+(?P<stat>Shots?(?:\s+On\s+Target)?)$",
        re.I,
    ),
]
...
def canonical_count_threshold(mode, raw_count):
    """Convert William Hill wording into the shared site ladder.
...
def match_inline_shot_row(label):
    label = clean(label)
    for index, pattern in enumerate(INLINE_SHOT_ROW_PATTERNS):
...
def is_inline_shot_selection_label(label):
    return match_inline_shot_row(label) is not None
...
def looks_like_player_name(s):
    ...
    if is_probably_heading(s):
        return False
    if is_inline_shot_selection_label(s):
        return False
    ...
    for idx in idxs:
        ...
        for j in range(idx + 1, min(idx + max_len, len(lines))):
            ...
            if (
                ...
                and not is_inline_shot_selection_label(tok)
            ):
                break
...
def parse_inline_shot_rows(block, key, market_name, prop_type):
    """Parse William Hill's full-label shot rows and canonicalise thresholds.

    William Hill does not display these ladders like most books. A row can be:
      Iqraam Rayners At Least 1 Shot On Target  EVS
      Iqraam Rayners Over 1 Shot On Target      9/2
      Iqraam Rayners Over 2 Shots On Target     12/1

    Those become the shared site selections 1+, 2+, 3+ respectively.
    """
    selections = []
    want_sot = key == "player_shots_on_target"

    for i, raw_label in enumerate(block):
        parsed = match_inline_shot_row(raw_label)
        if not parsed:
            continue

        stat_low = parsed["stat"].lower()
        is_sot = "on target" in stat_low
        if is_sot != want_sot:
            continue
        ...
        for j in range(i + 1, min(i + 4, len(block))):
            ...
            if is_odds(token):
                odds = token
                break
            if is_inline_shot_selection_label(token):
                break
...
def parse_player_market(lines, key):
    ...
    all_sels = []
    for block in blocks:
        if key in {"player_shots", "player_shots_on_target"}:
            # William Hill usually renders each shot ladder selection as one
            # complete row label (for example, "Over 1 Shot On Target").
            # Parse that format first and store canonical 1+/2+/3+/4+ values.
            all_sels.extend(parse_inline_shot_rows(block, key, market_name, prop_type))

            # Retain support for any fixture rendered as a conventional grid.
            all_sels.extend(parse_ladder_player_odds(block, market_name, prop_type))
        elif key in {"player_tackles", "player_fouls_committed", "player_fouls_won"}:
            all_sels.extend(parse_ladder_player_odds(block, market_name, prop_type))
...
def canonical_shot_mapping_warnings(markets):
    warnings = []
    for market in markets:
        if market.get("market") not in {"Player Shots", "Player Shots On Target"}:
            continue
        ...
        for selection in market.get("selections", []):
            ...
            if not match:
                warnings.append(
                    f"{market['market']}: non-canonical threshold {threshold!r} for {player or selection.get('selection', '')}"
                )
                continue
            ...
            if line != expected_line:
                warnings.append(
                    f"{market['market']}: {player} {threshold} has line {line!r}; expected {expected_line}"
                )

            if re.search(r"\b(?:at least|over)\s+\d+", player, re.I):
                warnings.append(
                    f"{market['market']}: raw William Hill label leaked into player field: {player!r}"
                )
    return warnings


def print_canonical_shot_samples(markets):
    for market_name in ["Player Shots On Target", "Player Shots"]:
...
def player_market_warnings(markets):
    warnings = canonical_shot_mapping_warnings(markets)
    for market in markets:
...
def scrape_match(page, fixture):
    ...
    print_canonical_shot_samples(markets)
