#!/usr/bin/env python3
"""
fetch_williamhill_worldcup_props_FAST_TEST3_V7_IMPACT_SUB.py

William Hill World Cup props scraper — FAST TEST3 V6 candidate — global William Hill player-stat row parsing.
...
Output:
  football/data/williamhill_worldcup_props_FAST_TEST3_V7_IMPACT_SUB.json
...
OUT_PATH  = ROOT / "football" / "data" / "williamhill_worldcup_props_FAST_TEST3_V7_IMPACT_SUB.json"
DEBUG_DIR = ROOT / "football" / "debug" / "williamhill_worldcup_props_FAST_TEST3_V7_IMPACT_SUB"
...
def parse_player_props(lines):
    ...
    # Primary parser for WH full-label count ladders. It does not depend on
    # fragile heading block boundaries.
    global_markets, _ = parse_global_inline_player_stat_markets(lines)
    ...
    # Only use the older heading-based parser when the global scan found zero
    # rows for an entire market. This protects genuine data while supporting a
    # possible conventional WH grid layout.
    for key in [
        "player_shots_on_target",
        "player_shots",
        "player_tackles",
...
def select_impact_sub_controls(page):
    """Explicitly select William Hill's Impact Sub price mode.

    William Hill can expose the same player-stat rows under both Impact Sub and
    Enhanced Win. We only want the Impact Sub price. This clicks every rendered
    Impact Sub control belonging to the currently expanded player markets before
    the page text is captured.
    """
    clicked = 0
    errors = 0

    try:
        # The arrow/icon is normally a separate element, so the text node itself
        # is exactly "Impact Sub". A narrow symbol-only fallback covers layouts
        # where the icon is included in accessible text without matching the
        # surrounding Impact Sub + Enhanced Win container.
        labels = page.get_by_text("Impact Sub", exact=True)
        count = labels.count()
        if count == 0:
            labels = page.get_by_text(
                re.compile(r"^\s*Impact Sub(?:\s*[^A-Za-z0-9\s])?\s*$", re.I)
            )
            count = labels.count()
    except Exception:
        return 0, 1

    # Work from the last control upwards. Clicking a tab can rerender its own
    # market card, so reverse order is less likely to disturb later locators.
    for i in range(count - 1, -1, -1):
        try:
            ...
            target = control.first if control.count() else label

            target.scroll_into_view_if_needed(timeout=1800)
    ...
    # Allow all affected price rows to finish rerendering before inner_text().
    if clicked:
        page.wait_for_timeout(700)
...
def capture_impact_sub_text(page, timeout):
    """Select Impact Sub on every open player market, then capture rendered text."""
    clicked, errors = select_impact_sub_controls(page)
    text = page.locator("body").inner_text(timeout=timeout)
    return text, clicked, errors
...
def scrape_match(page, fixture):
    ...
    try:
        ...
        impact_text, impact_clicked, impact_errors = capture_impact_sub_text(page, 18000)
        scroll_page(page, 5)
        chunks.append("=== TAB Popular/Default | PRICE MODE: IMPACT SUB ===\n" + impact_text)
        captured_tabs.append("Popular/Default")
        print(
            f"    ✓ Popular/Default ({clicked} relevant headings clicked; {len(present)} found; "
            f"Impact Sub selected on {impact_clicked} controls; {impact_errors} click errors)"
        )
    except Exception as exc:
    ...
    if player_tab:
        clicked, present = expand_relevant_player_markets_fast(page)
        impact_text, impact_clicked, impact_errors = capture_impact_sub_text(page, 20000)
        scroll_page(page, 7)
        chunks.append(f"=== TAB {player_tab} | PRICE MODE: IMPACT SUB ===\n" + impact_text)
        captured_tabs.append(player_tab)
        print(
            f"    ✓ {player_tab} ({clicked} relevant headings clicked; {len(present)} found; "
            f"Impact Sub selected on {impact_clicked} controls; {impact_errors} click errors)"
        )
    else:
    ...
    if click_tab(page, "Goals"):
        ...
        impact_text, impact_clicked, impact_errors = capture_impact_sub_text(page, 20000)
        scroll_page(page, 6)
        chunks.append("=== TAB Goals | PRICE MODE: IMPACT SUB ===\n" + impact_text)
        captured_tabs.append("Goals")
        print(
            f"    ✓ Goals ({clicked} relevant headings clicked; {len(present)} found; "
            f"Impact Sub selected on {impact_clicked} controls; {impact_errors} click errors)"
        )
    else:
    ...
    if not player_tab:
        ...
        if all_tab:
            ...
            impact_text, impact_clicked, impact_errors = capture_impact_sub_text(page, 20000)
            scroll_page(page, 7)
            chunks.append(f"=== TAB {all_tab} | PRICE MODE: IMPACT SUB ===\n" + impact_text)
            captured_tabs.append(all_tab)
            print(
                f"    ✓ {all_tab} fallback ({clicked} relevant headings clicked; {len(present)} found; "
                f"Impact Sub selected on {impact_clicked} controls; {impact_errors} click errors)"
            )
    ...
    # Show raw full-label row detection before deduplication. This tells us
    # whether a missing market is a page-capture problem or a parser problem.
    try:
        _, raw_stat_counts = parse_global_inline_player_stat_markets(lines_from_text(all_text))
        print("    raw canonical rows: " + ", ".join(
            f"{MARKET_META[key][0]}={raw_stat_counts.get(key, 0)}"
            for key in [
        ...
        print(f"    raw canonical row diagnostic failed: {exc}")
...
def main():
    ...
    print("=" * 60)
    print("William Hill World Cup Props Scraper — FAST TEST3 V7 IMPACT SUB ONLY")
