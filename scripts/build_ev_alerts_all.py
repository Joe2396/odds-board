...
def render_alert_card(alert, index):
    ...
    <article class="alert-card" data-sport="{esc(sport)}">
      ...
      <a class="btn-tracker"
         href="https://joe2396.github.io/odds-board/ufc/tracker/?sport={url_param(sport)}&event={url_param(alert.get('event',''))}&market={url_param(alert.get('market',''))}&selection={url_param(alert.get('selection',''))}&bookmaker={url_param(alert.get('bookmaker',''))}&odds={url_param(alert.get('odds',''))}"
         target="_blank">
        Add to Bet Tracker →
      </a>
    </article>
